...
public class UserStatus extends Activity {
	...
	private DateTime today, nextWeight, nextPhoto;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		today = dateInfo.parseDate(dateInfo.getDateFormat(today));
	...
	public void checkWeight() {
		nextWeight = dateInfo.parseDate(storage
				.getString("nextWeightDay", ""));
		if (nextWeight == null) {

		} else {

			// checks todays date against nextWeightDay in preferences to set
			// update
			// notifications
			if (today.equals(nextWeight)
					|| today.equals(nextWeight.minusDays(1))) {
				notifications += 1;
				boolean needWeight = true;

				storage.putBoolean("needWeight", needWeight);
				storage.putInt("notifications", notifications);
			}
		}
	}

	public void checkPhoto() {
		nextPhoto = dateInfo.parseDate(storage.getString("nextPhotoDay", ""));
		if (nextPhoto == null) {

		} else {

			if (today.equals(nextPhoto) || today.equals(nextPhoto.minusDays(1))) {
				notifications += 1;
				boolean needPhoto = true;

				storage.putBoolean("needPhoto", needPhoto);
				storage.putInt("notifications", notifications);
			}
		}
	}
